...
export type Evaluation={total:number;maxTotal:number;summary:string;provider:string;strengths?:string[];issues?:string[];improvements?:string[];dimensions:{key:string;name:string;criterion?:string;score:number;maxScore:number;evidence:string;suggestion:string}[]}
export type Submission={id:string;assignmentId:string;studentName:string;submittedAt:string;status:'queued'|'evaluating'|'graded'|'failed';progress:number;code:string;evaluation?:Evaluation}
